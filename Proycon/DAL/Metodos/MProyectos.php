...
class MProyectos implements IProyectos {
    ...
    public function MostrarPedidos($TipoPedido, $ID_Proyecto) {
        ...
        $sql = "SELECT Consecutivo,TipoPedido,Nombre,Fecha FROM tbl_boletaspedido tb, tbl_usuario tu WHERE tb.ID_Proyecto = ? AND tb.TipoPedido= ? and tb.ID_Usuario = tu.ID_Usuario  ORDER BY CONSECUTIVO DESC;";
        $stmt = $this->conn->prepare($sql);
